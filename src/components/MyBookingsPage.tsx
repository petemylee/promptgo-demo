...
export default function MyBookingsPage() {
  ...
  const buildReportFilename = () => {
    if (reportMonthFrom === reportMonthTo) return `travel_summary_${reportMonthFrom}.pdf`;
    return `travel_summary_${reportMonthFrom}_to_${reportMonthTo}.pdf`;
  };

  const openMonthlyReportPreview = async () => {
    ...
    try {
      const previewUrl = `/api/my/bookings/report/pdf?from=${encodeURIComponent(reportMonthFrom)}&to=${encodeURIComponent(
        reportMonthTo
      )}&preview=1`;
  ...
  const downloadMonthlyReportPdf = async () => {
    ...
    try {
      const res = await fetch(
        `/api/my/bookings/report/pdf?from=${encodeURIComponent(reportMonthFrom)}&to=${encodeURIComponent(reportMonthTo)}`
      );
      if (!res.ok) {
        const err = await res.json().catch(() => null);
        ...
        <div className="mb-6 rounded-2xl bg-white/90 p-5 shadow ring-1 ring-black/5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-600">เลือกช่วงเดือนที่ต้องการ แล้วดูตัวอย่างก่อนดาวน์โหลด PDF</p>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
              ...
              <button
                type="button"
                onClick={openMonthlyReportPreview}
